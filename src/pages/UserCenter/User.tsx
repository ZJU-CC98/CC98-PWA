...
export default withStyles(styles)(class extends React.Component<Props & { classes: any }, State>{
  state: State = {
    recentTopics: [],
    isLoading: false,
    isEnd: false,
  ...
  getRecentTopics = async () => {
    ...
    if (info) {
      const recentTopicsData = await GET<ITopic[]>(`/user/${info.id}/recent-topic?from=${from}&size=10`);
      recentTopicsData
        .fail()
        .succeed(
          async (newRecentTopics: ITopic[]) => {
            newRecentTopics.forEach(
              async (topic) => {
    ...
    <ListItem>
      <div className={TopicItemRootStyle}>
        <div className={TopicItemMetaStyle}>
          <div onClick={()=>navigate(`/board/${props.data.boardId}`)}className={TopicItemBoardStyle}>{props.data.boardName}</div>
          <div className={TopicItemTimeStyle}>{new Date(props.data.time).toLocaleString()}</div>
        </div>
        <div className={TopicItemTitleStyle} onClick={()=>navigate(`/topic/${props.data.id}`)}>{props.data.title}</div>
      </div>
    </ListItem>
  )
